...
async function _ensureOutputProduct(r) {
    ...
    // Per-unit macros: total raw-ingredient macros ÷ output_qty.
    // weight_g = 100 makes "1 ud == 100g equivalent" so the backend formula
    //   kcal = qty * weight_g/100 * kcal_100g
    // collapses to kcal_per_ud = kcal_100g.
    ...
    if (linked) {
        // Refresh macros in case the recipe (or the macro formula) changed since creation.
        try {
            await window.apiCall(`/products/${r.output_product_id}`, 'PATCH', {
                kcal_100g: per.kcal,
                proteins_100g: per.p,
                carbs_100g: per.c,
                fat_100g: per.fat,
            ...
            console.warn('No se pudo refrescar macros del producto cocinado', e);
        }
        return r.output_product_id;
    }
