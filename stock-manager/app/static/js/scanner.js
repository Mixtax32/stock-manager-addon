...
window.stopAllCameras = async () => {
    ...
    if (window.html5QrCode) {
        try { await window.html5QrCode.stop(); } catch(e) { console.debug("Scanner QR ya estaba detenido:", e); }
    }
};
...
window.startTicketScanner = async () => {
    ...
    try {
        window.currentTicketStream = await navigator.mediaDevices.getUserMedia({ 
            video: { facingMode: "environment", width: { ideal: 1920 }, height: { ideal: 1080 } } 
        });
        const video = document.createElement('video'); 
        video.srcObject = window.currentTicketStream; 
        ...
        captureBtn.onclick = async () => {
            ...
            if (window.currentTicketStream) { 
                window.currentTicketStream.getTracks().forEach(t => t.stop()); 
                window.currentTicketStream = null; 
            }
            container.innerHTML = ''; 
            document.getElementById('start-scan')?.classList.remove('hidden'); 
...
window.onScanSuccess = async (decodedText) => {
    ...
    if (p) {
        ...
        document.getElementById('existing-product-name').textContent = p.name;
        document.getElementById('serving-size').value = p.serving_size || '';
        document.getElementById('existing-product-stock').textContent = `(${p.stock} ${p.unit_type || 'uds'})`;
        window.renderScanImage(p.image_url);
        window.renderExistingBatches(p);
    } else {
        ...
        try {
            ...
            if (barcodeData && barcodeData.found) {
                window.currentScannedImageUrl = barcodeData.image_url;
                window.renderScanImage(barcodeData.image_url);
        ...
        } catch (e) { console.error(e); window.renderScanImage(null); }
    }
    ...
    container.innerHTML = product.batches.map(b => {
        ...
        return `<div class="existing-batch-row" style="border-bottom: 1px solid #222; padding: 8px 0; display: flex; justify-content: space-between; align-items: center;">
            ...
            <div style="display: flex; gap: 6px;">
                <button class="btn-round" onclick="window.updateBatchDelta(${b.id}, -1)">-</button>
                <button class="btn-round" onclick="window.updateBatchDelta(${b.id}, 1)">+</button>
            </div>
        </div>`;
    }).join('');
...
window.processTicketImage = async (canvas) => {
    ...
    try {
        ...
        window.currentTicketItems = window.parseTicketText(result.data.text, window.products);
        window.showTicketResults(window.currentTicketItems);
    } catch (err) { progress.classList.add('hidden'); window.showToast('Error lector: ' + err.message, 'error'); }
};
